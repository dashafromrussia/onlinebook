...
import { addHistory } from '../store/actions/post'


export const CreateHistoryScreen= ({navigation}) => {
...
const saveAll =()=>{
   ...
   navigation.navigate('Histories')
    ...
    return(
    <ScrollView>
      <Text>Создайте историю в виде диалога для лучшего восприятия языка.Вставьте задания по ходу истории для проверки внимательности учеников.</Text>
    <TextInput placeholder="Название истории..." value={namehist} width={230} onChangeText={setNamehist}/>
      <Button title="Добавить блок истории" color="black" onPress={addBlock}/>
      {blockmass.map(el=><HistoryBlocks key={el} id={el} saveBlock={saveBlock}/>)}
